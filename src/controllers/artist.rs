...
pub async fn count_artists(
    ...
    let count = ArtistRepositoryImpl::count(&state, access.user().id).await?;
    Ok(Json(count))
}
...
#[utoipa::path(
    ...
    responses(
        (status = 200, body = Vec<ArtistPublic>)
    )
)]
pub async fn find_all_artists(
    ...
    let artists = ArtistRepositoryImpl::find_all(&state, access.user().id).await?;
    Ok(Json(artists))
}
...
pub async fn find_artist_by_id(
    ...
    match ArtistRepositoryImpl::find_by_id(&state, id, access.user().id).await? {
...
pub async fn create_artist(
    ...
    let artist = ArtistRepositoryImpl::create(&state, &payload, access.user().id).await?;
...
pub async fn update_artist(
    ...
    payload.validate()?;
    artist_exists(&state, payload.id, access.user().id).await?;
